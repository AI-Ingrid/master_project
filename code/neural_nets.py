...
class NavigationNet(nn.Module):
    def __init__(self, num_memory_nodes, num_features, num_stacked_LSTMs, num_frames_in_stack, num_airway_segment_classes, num_direction_classes, batch_size, frame_dimension):
        super().__init__()
        self.num_airway_segment_classes = num_airway_segment_classes
        ...
        self.lstm = nn.LSTM(input_size=self.input_size, hidden_size=self.num_memory_nodes,
                            num_layers=self.num_stacked_LSTMs, batch_first=True)
    ...
    def forward(self, X, hidden):
        ...
        # LSTM
        X, (self._hidden_state, self._hidden_cell) = self.lstm(X, hidden)  # [batch_size, num_frames_in_stack, output_size]
...
def create_neural_net(num_memory_nodes, num_features_extracted, model_type, num_frames_in_stack, num_airway_segment_classes, num_direction_classes, frame_dimension, batch_size):

    print("-- NEURAL NET --")
    # Baseline -> No LSTM and no classifying of direction
    if model_type == 'baseline':
        neural_net = Baseline(num_features=num_features_extracted, num_frames_in_stack=num_frames_in_stack,
                              num_airway_segment_classes=num_airway_segment_classes,
                              batch_size=batch_size, frame_dimension=frame_dimension)
    # Alpha -> 1 LSTM cell
    elif model_type == 'blomst' or model_type == 'alpha':
        neural_net = NavigationNet(num_memory_nodes=num_memory_nodes, num_features=num_features_extracted, num_stacked_LSTMs=1,
                                   num_frames_in_stack=num_frames_in_stack, num_airway_segment_classes=num_airway_segment_classes,
                                   num_direction_classes=num_direction_classes, batch_size=batch_size, frame_dimension=frame_dimension)
    # Beta-> 2 LSTM cells
    elif model_type == 'boble':
        neural_net = NavigationNet(num_memory_nodes=num_memory_nodes, num_features=num_features_extracted, num_stacked_LSTMs=2,
                                   num_frames_in_stack=num_frames_in_stack, num_airway_segment_classes=num_airway_segment_classes,
                                   num_direction_classes=num_direction_classes, batch_size=batch_size, frame_dimension=frame_dimension)
    # Gamma -> 3 LSTM cells
    elif model_type == 'belle':
        neural_net = NavigationNet(num_memory_nodes=num_memory_nodes, num_features=num_features_extracted, num_stacked_LSTMs=3,
                                   num_frames_in_stack=num_frames_in_stack, num_airway_segment_classes=num_airway_segment_classes,
                                   num_direction_classes=num_direction_classes, batch_size=batch_size, frame_dimension=frame_dimension)
